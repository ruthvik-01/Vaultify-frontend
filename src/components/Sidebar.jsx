...
export default function Sidebar({ onClose }) {
  ...
  return (
    <aside className={`${isCompact ? 'w-20 p-4 items-center' : 'w-72 p-6'} h-screen sticky top-0 bg-brand-cream-dark border-r border-brand-sand/80 flex flex-col justify-between overflow-y-auto z-30 shrink-0 transition-all duration-300`}>
      {/* Top Header */}
      <div className={isCompact ? 'w-full flex flex-col items-center' : ''}>
        <div className={`flex items-center ${isCompact ? 'justify-center' : 'justify-between'} mb-8 w-full`}>
          <div className="flex items-center space-x-3">
            <div className="bg-brand-olive text-brand-cream p-2.5 rounded-xl shadow-sm cursor-pointer" title="Vaultify Dashboard">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={`
                  flex items-center ${isCompact ? 'justify-center p-3 w-12 h-12' : 'space-x-3 px-4 py-3'} rounded-xl text-sm font-semibold transition-all group duration-200 transform
                  ${isActive 
